-- =============================================================================
-- Session 5: Fix validate_risk_allocation in supplier_config_pkg
--
-- PROBLEM (current code):
--   Sums ALL active risk_rule_config weights and requires they equal exactly 100.
--   This is architecturally wrong because:
--     - BASE component rules are additive and must total <= 55
--     - DUPLICATE component rules are "take the strongest single match" and no
--       individual rule should exceed 45 (they are never summed)
--     - Requiring sum = 100 makes it impossible to configure the system correctly
--       e.g. EXACT_TAX_ID_MATCH=45 + EXACT_BANK_MATCH=45 alone = 90,
--       before any BASE rules are added.
--
-- FIX:
--   Replace the single sum=100 check with two independent architectural checks:
--     1. Sum of active BASE component weights <= 55
--     2. No individual active DUPLICATE component weight > 45
--
--   update_risk_rule() already calls validate_risk_allocation() after every
--   change, so the new check will be enforced on every admin edit automatically.
--
-- Scope: only supplier_config_pkg body is changed. Spec is unchanged.
-- Deploy: replace the full package body in ATP_PACKAGE_BODIES.sql AND apply
--         this standalone patch script to update the running database.
-- =============================================================================
...
create or replace package body supplier_config_pkg as
  ...
  -- ---------------------------------------------------------------------------
  -- FIXED: validate_risk_allocation
  --
  -- Old behaviour: required sum(all active weights) = 100 — architecturally wrong.
  --
  -- New behaviour (matches design spec):
  --   Rule 1 — BASE component: sum of all active BASE rule weights must be <= 55
  --            (BASE rules are additive; max total contribution is 55 points)
  --   Rule 2 — DUPLICATE component: no single active DUPLICATE rule weight may
  --            exceed 45 (only the strongest matching rule is ever applied;
  --            they are never summed, so any single rule > 45 would exceed the cap)
  --
  -- Called by: update_risk_rule() after every admin edit
  -- ---------------------------------------------------------------------------
  procedure validate_risk_allocation is
    l_base_total   number;
    l_dup_max      number;
  begin
    -- Rule 1: sum of active BASE weights
    select nvl(sum(weight_points), 0)
      into l_base_total
      from risk_rule_config
    ...
    -- Rule 2: largest single active DUPLICATE weight
  ...
  begin
    ...
    -- Weight bounds: 0–100 is the raw column constraint; architectural
    -- bounds (55 for BASE, 45 for DUPLICATE) are enforced by validate_risk_allocation below.
    if p_weight < 0 or p_weight > 100 then
      raise_application_error(-20022, 'Risk rule weight must be between 0 and 100');
    end if;
    ...
    -- Enforce architectural bounds after the change.
    -- If this raises, the calling transaction is rolled back automatically.
  ...
  begin
    ...
    if not regexp_like(l_country_code, '^[A-Z]{2}$') then
      raise_application_error(-20023, 'Country code must be ISO alpha-2 format');
    end if;
